    ...
    <>
      <Layer>
        {[...Array(Math.ceil(props.canvasHeight/GRID_SIZE))].map((_, i) => (
          <Line key={i} 
            points={[0, (i + 1) * GRID_SIZE, props.canvasHeight, (i + 1) * GRID_SIZE]}
            dash={[2, 2]}
        ...
        {[...Array(Math.ceil(props.canvasHeight/GRID_SIZE))].map((_, i) => (
          <Line key={i} 
            points={[(i + 1) * GRID_SIZE, 0, (i + 1) * GRID_SIZE, props.canvasWidth]}
            dash={[2, 2]}
            stroke={i == (Math.ceil(props.canvasHeight/GRID_SIZE)/2) ? basePalette.primary.main : basePalette.grey[400]}
            strokeWidth={i == (Math.ceil(props.canvasHeight/GRID_SIZE)/2) ? 2 : 1} />
        ))}
      </Layer>
      <Layer key={"Meter"}>
